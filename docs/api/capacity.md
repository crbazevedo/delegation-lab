...
Each node's effective skill depends on its parents' corrected quality (Eq. 7).
The function walks the DAG in topological order, applying the prior-aware
recursive formula (Eq. 11) at each node. The default `sigma_0=0.0` preserves
the conservative examples from the paper.
